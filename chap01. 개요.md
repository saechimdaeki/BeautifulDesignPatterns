...
코드 설계의 중요성을 깨닫고 의도 적으로 코드 설계를 의식하고 개발하면 복잡한 코드 설계와 개발을 할때도 점점 더 쉽게 문제를 해결할 수 있을 것이다
...
코드의 명이 의미가 있는지, 주석이 자세히 기술되어 있는지, 함수 길이는 적절한지, 모듈 구분이 명확한지, 코드가 높은 응집도와
...
단위 테스트를 작성하기 어렵다는 뜻이며, 기본적으로 코드의 설계에 문제가 있음을 보여준다
...
설계 원칙은 정신적인 방법이고 디자인 패턴은 움직임이다. 따라서 설계 원칙은 디자인 패턴보다 더 보편적이고 중요하다.
...
어렵거나 주어진 이유가 터무니 없다면 이것은 과도한 설계이며 설계를 위한 설계라고 결론을 내릴 수 있다
